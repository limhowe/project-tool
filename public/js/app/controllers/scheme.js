...
.controller('schemeController', ['$scope', '$rootScope', '$stateParams', 'Project', 'Node', '$timeout', 'Scheme','$rootScope', '$modal', 'User', 'socketService', '$i18next',
function ($scope, $rootScope, $stateParams, Project, Node, $timeout, schemeService, $rootScope, $modal, User, socketService, $i18next) {
...
function doubleClickListener(d) {
  ...
  if (scale && scale < 1){
    alert("You will not be able to double click to edit because the chart is zoom. To edit, return to default zoom level.")
    return
  }
  ...
  inputDom.save = function() {
    ...
    d.title = input.value;
    $scope._active_form = false;
    Node.update( d._id, __node )
    .then(function (node_updated) {
    })
    d3.selectAll('#title_of_'+d._id)
    .text(function (d) {
        if (d == root) d.title = d.name
        if($rootScope.getMixedStrLength(d.title) > 15){
          return $rootScope.subStringMixedStr(d.title,0,15) + '...';
        }else{
          return d.title;
        }
      })
...
function calculateListY(d, i){
  ...
  var normalCase = d.parent ? (!isNaN(d.parent.y) ? d.parent.y : 0) + ( ( 50 ) * ( d.lvling ) ): null;
  var ghostCase = d.parent ? (!isNaN(d.parent.y) ? d.parent.y : 0) + 50 : (!isNaN(d.y) ? d.y : 0);
    ...
    if (side === 'bottom') {
      ...
      Node.add( $scope.project.id, parentId, newNode )
        .then( function(data){
          ...
          Node.changePosition(data._id, insertIndex+1);
      ...
      Node.add( $scope.project.id, parentId, newNode )
        .then( function(data){
          ...
          newNode._id = data._id;
          var insertIndex = d.parent.children.indexOf(d);
          Node.changePosition(data._id, insertIndex)
...
function deleteNode(d, root) {
  var answer = confirm($i18next('wbsView.deleteTaskConfirm'));
  if (answer){
    Node.delete(d._id)
    .then(function (){
      d.parent.children = d.parent.children.filter(function (elm) {
        return elm.id != d.id;
      });
      update(root)
    })
...
function sortNodes(root){
  if (root._nodes) {
    root._nodes.sort(function(a, b) { return d3.ascending(a.position, b.position); })
    root._nodes.forEach(function(_node){
      sortNodes(_node)
    })
  };
}
  ...
  .then( function (project) {
    ...
    User.me().then(function(user){
      ...
      Node.getList($stateParams.id).then(function (__nodes) {
        ...
        root.y0 = 0;
        recursiveWalk(root, function(node){
          if (node && node.isListParent && node.children && node.children.length > 0) {
            node.children.forEach(function(child){
              child.isList = true
            })
          };
        })
        $scope.$on('task_title_edited', function(a, b){
          d3.selectAll('#title_of_'+b._id)
          .text(function (d) {
            if($rootScope.getMixedStrLength(b.text) > 15){
                return $rootScope.subStringMixedStr(b.text,0,15) + '...';
             }else{
                 return b.text;
            }
          })
...
function updateWBS() {
  $('.ohoo .draw-area-container svg').remove();
  $scope.showLoader = true;
...
socket.on('raci.add', function(info) {
  ...
  if (_.isUndefined($rootScope.currentNode.racis)) {
    $rootScope.currentNode.racis = [];
  }

  var found = _.find($rootScope.currentNode.racis, function (raci) {
    return info.raci._id == raci._id;
  });

  if (!found) { $rootScope.currentNode.racis.push(info.raci); }
});
...
function update(source, doubleClickTarget, resolving) {
  ...
  // title text
    ...
    .attr('id', function(d){ return 'title_of_'+d._id })
    .text(function (d) {
      if (d == root) d.title = d.name
      if($rootScope.getMixedStrLength(d.title) > 15)
         return $rootScope.subStringMixedStr(d.title,0,15) + '...';
    ...
    .style('text-decoration', function(d){
      return d.complete == 100 ? 'line-through' : 'none';
    })
    .style('font-style', function(d){
      return d.complete == 100 ? 'italic' : 'normal';
    })
    .style('color', '#fff')
    ...
    nodeEnter.filter(function(d){
      ...
      .on('click', function(d){
          $scope.modalInstance = $modal.open({
            templateUrl: 'schemeAssignModal.html',
            controller: 'schemeAssignController'
          });
          $rootScope.schemeNode = d;
...
$scope.$on('child_task_created', function(e, task, side){
  var fun = addNode(side);
  fun(task);
})

$scope.$on('task_delete', function(e, task){
  $rootScope.hideTaskForm()
  if ( task.children && task.children.length > 0 || task._children && task._children.length > 0 ) {
    $scope.showDeleteModal(task)
  }
  else{
    deleteNode(task, root)
  }
})

$scope.$on('task_complete_updated', function(e, task){
  d3.select('#title_of_'+task._id )
    .style('text-decoration', function(d){
      return d.complete == 100 ? 'line-through' : 'none';
    })
    .style('font-style', function(d){
...
.controller('schemeDeleteController', ['$scope', '$rootScope', 'Node', 'node', '$modalInstance',
function ($scope, $rootScope, Node, node, $modalInstance) {
...
.controller('schemeAssignController', ['$scope', '$rootScope', '$timeout', '$modalInstance', 'commonService',
function ($scope, $rootScope, $timeout, $modalInstance, commonService) {

  $scope.close = function() {
    $modalInstance.close();
  };

  $scope.matchUser = function (user, assigned_user) {
    var result = false;
    if (user.invite_email && assigned_user.invite_email && user.invite_email == assigned_user.invite_email) {
      result = true;
    } else if( !user.invite_email && !assigned_user.invite_email && user.user && user.user._id == assigned_user.user ){
      result = true;
    }
    return result;
  };

   $scope.generate_user_dropdown = function () {
    $scope.project.users.forEach(function (user) {
      user.email = user.invite_email || user.user.email;
      if (!node.assigned_users || !node.assigned_users.length) {
        $scope.users = $scope.project.users;
      } else {
        node.assigned_users.forEach(function (assigned_user) {
          if (!$scope.matchUser(user, assigned_user) && $scope.users.indexOf(user) == -1) {
  ...
  var node = $scope.$root.schemeNode;
  commonService.manageUsers(node, $scope.project.users);
  $scope.users = [];
  $scope.assigned_users = [];
  if (!node.assigned_users) node.assigned_users = [];

  $scope.generate_user_dropdown();

  $timeout(function(){
    $scope.avatars = node._assigned_users;
  }, 0)

  $scope.selected_user = {};

  // big crap here with passing parameters
  $scope.broadCast_assign = function (assigned_user) {
    var can = true;

    node.assigned_users.forEach(function (user) {
      if (user.invite_email && assigned_user.invite_email && user.invite_email == assigned_user.invite_email) {
        alert('user is already assigned');
        $scope.selected_user = {};
        can = false;
      } else if( !user.invite_email && !assigned_user.invite_email && user.user && user.user == assigned_user.user._id){
        alert('user is already assigned');
        can = false;
      }
    });

    $scope.$root.schemeNode = null;

    if (can) {
      $scope.$root.$broadcast('user_assigned_from_assign_modal', assigned_user, node);
      $modalInstance.close();
    } else {
      $modalInstance.close();
      $scope.selected_user = {};
  ...
  $scope.broadCast_reject = function (assigned_user) {
    $timeout(function () {
      node.assigned_users.splice(node.assigned_users.indexOf(assigned_user));
      commonService.manageUsers(node, $scope.project.users);
      $scope.avatars = node._assigned_users;
      $scope.generate_user_dropdown();
    }, 0);
    var email = assigned_user.invite_email || assigned_user.user.email;
    $scope.$root.$broadcast('user_rejected_from_assign_modal', email, node);
  };
}])
