...
angular.module('App.directives', ["kendo.directives"])
  ...
  .directive('clickOutside', ['$document', function clickOutside($document) {
    return {
        restrict: 'A',
        scope: {
            clickOutside: '&'
        ...
        link: function ($scope, elem, attr) {
            ...
            var button = attr.outside-button;
            if (button) classList.push(button)
  ...
  .directive('clickOff', ['$document', function clickOff($document) {
    return {
        restrict: 'A',
        scope: {
            clickOff: '&'
        ...
        link: function ($scope, elem, attr) {
          ...
          //   var $elem = $(elem);
          //   var $target = $(e.target);
          //   if (!$(e.target).closest($elem).length && $(attr.clickButton) != $target){
          //     if ($scope.timeout) {
          //       clearTimeout($scope.timeout)
          //     }
          //     $scope.timeout = setTimeout(function() {
          //       $document.off('click', bindEvent)
          //       $scope.$eval($scope.clickOff);
          //     }, 50);
