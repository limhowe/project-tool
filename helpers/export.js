...
exports.csv_prepare_json = function (doc, tree, level_text) {
  ...
  for (i = 0; i < tree.length; i++) {
    ...
    if (node.$expanded !== false && node._nodes && node._nodes.length) {
...
exports.risk_prepare_json = function (risks) {
  ...
  risks.forEach(function (risk) {
    result.push({
      ...
      'Associated Task': risk.task ? risk.task.title : ''
    });
  });
...
exports.handler = function (export_type, view_type, project_id, tree, callback) {
  ...
  } else if (view_type == 'detailed') { // detailed
    if (export_type === 'pdf') {
      Models.Project.findById(project_id, function (error, project) {
        if (error) return callback(error);
  ...
  } else if (view_type == 'tasks') {
    if (export_type == 'csv') {
      prepareTaskJson(doc, tree);

      var data = {
        data: doc,
        fields: [
          'TaskNumber', 'TaskTitle', 'Assignee', 'StartDate', 'EndDate',
          'Notes', 'Complete%', 'Duration', 'Cost', 'Resources'
        ]
      };
      ...
      json2csv(data, function (err, csv) {
        ...
        amazon
          .upload('projects', 'csv-task', project_id, csv)
          .then(function (url) {
            callback();
          })
          .catch(function (error) {
...
function prepareTaskJson(doc, tasks) {
  _.each(tasks, function (task) {
    ...
    if (task.assigned_users && task.assigned_users.length) {
      _.each(task.assigned_users, function (assignee) {
        if (!assignee.user) {
          return;
        }
        assignedUsers.push(assignee.user.username);
      });
    }
    ...
    doc.push({
      ...
      'Assignee': assignedUsers.join(', '),
      'StartDate': task.start_date,
      'EndDate': task.end_date,
      'Notes': task.notes,
      'Complete%': task.complete,
      'Duration': task.duration.value + task.duration.type + '(s)',
      'Cost': task.cost,
      'Resources': task.resources
    });

    if (task.$expanded !== false && task._nodes && task._nodes.length) {
      prepareTaskJson(doc, task._nodes);
    }
  });
}
